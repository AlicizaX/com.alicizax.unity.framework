...
namespace AlicizaX.Resource.Runtime
{
    ...
    class RemoteServices : IRemoteService
    {
        ...
        {
            string primary = string.Concat(_defaultHostPrefix, fileName);
            return _fallbackHostPrefix == null
                ? new[] { primary }
                : new[] { primary, string.Concat(_fallbackHostPrefix, fileName) };
        }
        ...
        private static string NormalizeHostPrefix(string hostServer)
        {
            ...
            return hostServer[hostServer.Length - 1] == '/' ? hostServer : string.Concat(hostServer, "/");
        }
    }
